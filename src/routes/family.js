...
router.post('/login', (req, res) => {
  const { phone: rawPhone, student_name } = req.body;
  if (!rawPhone) return res.status(400).json({ success: false, message: '전화번호를 입력해주세요.' });
  // 하이픈/공백 제거 후 정규화 (010-1234-5678 → 01012345678)
...
router.get('/reservations', auth, familyAuth, (req, res) => {
  ...
  const family = db.prepare('SELECT student_id FROM family_members WHERE id=?').get(req.user.id);
...
router.post('/reservations', auth, familyAuth, (req, res) => {
  ...
  const family = db.prepare(`SELECT f.student_id, s.classroom_id, c.kindergarten_id FROM family_members f JOIN students s ON s.id=f.student_id JOIN classrooms c ON c.id=s.classroom_id WHERE f.id=?`).get(req.user.id);
